...
CREATE TABLE users (
                       id SERIAL PRIMARY KEY,
                       tenant_id INT REFERENCES tenants(id) ON DELETE CASCADE,
                       role INT REFERENCES users_role(id),
...
CREATE TABLE orders (
                        ...
                        status VARCHAR(50) DEFAULT 'PENDING' CHECK (status IN ('PENDING', 'IN_PROGRESS', 'READY','DELIVERED')), -- 'PENDING', 'IN_PROGRESS', 'READY', 'DELIVERED'
                        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
